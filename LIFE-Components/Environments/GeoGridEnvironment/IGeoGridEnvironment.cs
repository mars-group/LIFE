...
namespace LIFE.Components.Environments.GeoGridEnvironment
{
    public interface IGeoGridEnvironment<T> : ILifeAutoInitialized where T : IGeoCoordinate
    {
        ...
        IEnumerable<T> Explore(double latitude, double longitude, double maxDistanceInM = -1,
            int maxNumberOfResults = -1, Predicate<T> predicate = null);

        IEnumerable<T> Explore(IGeoCoordinate gpsCoordinate, double maxDistanceInM = -1, int maxNumberOfResults = -1,
