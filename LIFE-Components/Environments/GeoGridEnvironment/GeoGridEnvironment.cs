...
namespace LIFE.Components.Environments.GeoGridEnvironment
{
    public class GeoGridEnvironment<T> : IGeoGridEnvironment<T> where T : IGeoCoordinate
    {
        ...
        (IGeoCoordinate gpsCoordinate, double maxDistanceInM = -1, int maxNumberOfResults = -1,
            Predicate<T> predicate = null)
        {
            return Explore(gpsCoordinate.Latitude, gpsCoordinate.Longitude, maxDistanceInM, maxNumberOfResults,
                predicate);
        }

        public T GetNearest(double latitude, double longitude, double maxDistanceInM = -1,
            Predicate<T> predicate = null)
        {
            var cell = GetCellForGps(latitude, longitude);
