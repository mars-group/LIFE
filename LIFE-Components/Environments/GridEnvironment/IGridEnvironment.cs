...
namespace LIFE.Components.Environments.GridEnvironment
{
    public interface IGridEnvironment<T> where T : IEquatable<T>, IGridCoordinate
    {
        ...
        IEnumerable<T> Explore(int x, int y, int maxDistanceInCells = -1, int maxNumberOfResults = -1, Predicate<T> predicate = null);

        IEnumerable<T> Explore(IGridCoordinate gridCoordinate, int maxDistanceInCells = -1, int maxNumberOfResults = -1, Predicate<T> predicate = null);
        ...
        /// <summary>
        /// Moves <param>objectToMove</param> to destination
        /// along an optimal path with minimal costs.
        /// Note that his method does not guarantee to reach <param>xDestination</param>,<param>yDestination</param>
        /// </summary>
        ...
        /// <returns>T with updated coordinates.</returns>
        T MoveTowardsTarget(T objectToMove, int xDestination, int yDestination, int distance);
        T MoveTowardsTarget(T objectToMove, IGridCoordinate destination, int distance);
