...
namespace LIFE.Components.ESC.SpatialAPI.Entities.Transformation
{
    public struct Vector3 : IEquatable<Vector3>
    {
        ...
        {
            get { return GetDistance(Zero, this); }
        }

        public static Vector3 Random
        {
            get { return new Vector3(random.Next(), random.Next(), random.Next()); }
        }

        public static Vector3 Cube(double value)
        {
        ...
        {
            return Compare(X, other.X) && Compare(Y, other.Y) && Compare(Z, other.Z) && _isSet.Equals(other._isSet);
        }
        ...
        {
            if (Math.Abs(length) < 0.0001) return Zero;

            var cur = ComputeMagnitude();
            if (Math.Abs(cur) < 0.0000001) throw new Exception("Attempting to normalize a zero vector");

            var coeff = length / cur;
