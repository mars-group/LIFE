    ...
    public class SensorArray
    {
        ...
        public TResult Get<TResult>(string sensorId)
        {
            if (_results.ContainsKey(sensorId))
            {
                ...
                throw new SensorReturnsOtherTypeException("[SensorArray] Cast error: '" + sensorId +
                                                          "' does not return " + typeof(TResult) + "!");
            }
        ...
        public void DebugSensors()
        {
            foreach (var sensor in _sensors.Keys)
            {
                Console.WriteLine("<" + sensor.GetType() + ">: '" + _sensors[sensor] + "' ==> " +
                                  _results[_sensors[sensor]]);
            }
        }
    }
