...
namespace LIFE.Components.Services.AgentShadowingService.Implementation
{
        ...
        where TServiceInterface : class
    {
        ...
        private void OnRemoveShadowAgentMessageReceived(object sender, RemoveShadowAgentEventArgs e)
        {
            ...
            if (handler != null)
                handler(this, new LIFEAgentEventArgs<TServiceInterface>(
        ...
        public TServiceInterface ResolveAgent(Guid agentId)
        {
            ...
            if (serviceapp != null &&
                serviceapp.ContainsService<TServiceInterface, TServiceClass>(agentId, _typeOfServiceInterfaceName))
            {
                return serviceapp.GetServiceByID<TServiceInterface, TServiceClass>(agentId,
                    _typeOfServiceInterfaceName);
            }
