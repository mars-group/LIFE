...
namespace SimulationController.Interface
{
    ...
    public class SimulationControllerNodeJSInterface
    {
        ...
        public async Task<object> GetSimController(dynamic input) {
			...
			return new {
				getAllModels = (Func<object,Task<object>>)(async (i) => {
					return await Task.Run(() => simController.GetAllModels ());
				}),
				getConnectedNodes = (Func<object, Task<object>>)(async (i) => {
					return await Task.Run(() => simController.GetConnectedNodes ());
				})
			};
    }
}
