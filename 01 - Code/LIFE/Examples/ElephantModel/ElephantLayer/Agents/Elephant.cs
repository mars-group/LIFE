...
public class Elephant : IAgent, IQuadObject{
    ...
    public Elephant(float x, float y, Size size, double direction, int visionDistance, double stepLength, PlantLayerImpl plantLayer, WaterLayerImpl waterLayer, ITwoDimEnvironment<Elephant> twoDimEnv){
        _bounds.X = x;
		_bounds.Y = y;
		_bounds.Size = size;
    ...
    private double GetWaterholeDirection(TWaterhole actor) {
        double angle = getTurnAngleTo(new Location() { X = actor.Bounds.X + actor.Bounds.Width / 2, Y = actor.Bounds.Y - actor.Bounds.Height / 2 });
	...
	private double GetWaterholeFactorWeight(TWaterhole actor) {
		if(!isThirsty)
			return 0;
        double distance = distanceTo(new Location() { X = actor.Bounds.X + actor.Bounds.Width / 2, Y = actor.Bounds.Y - actor.Bounds.Height / 2 });
