...
namespace ASC.Communication.Scs.Communication.Protocols.BinarySerialization {
    ...
    public class BinarySerializationProtocol : IScsWireProtocol {
        ...
        private bool ReadSingleMessage(ICollection<IScsMessage> messages) {
            ...
            var remainingLength = _receiveMemoryStream.Length - (4 + messageLength);
            if (remainingLength > 0) {
                var remainingBytes = ReadByteArray(_receiveMemoryStream, (int) remainingLength);
