...
namespace Daylight {
    ...
    public class ID : IComparable {
        ...
        public ID(byte[] data) {
            ...
            else throw new Exception("An ID must be exactly " + ID_LENGTH + " bytes.");
        }
        ...
        public static bool operator <(ID a, ID b) {
            for (int i = 0; i < ID_LENGTH; i++) {
                if (a.data[i] < b.data[i]) return true; // If first mismatch is a < b, a < b
                if (a.data[i] > b.data[i]) return false; // If first mismatch is a > b, a > b
            }
        ...
        public override bool Equals(object obj) {
            if (obj is ID) return this == (ID) obj;
            return false;
        }
        ...
        public ID RandomizeBeyond(int bit) {
            ...
            for (int i = bit + 1; i < 8*ID_LENGTH; i++) {
                if (rnd.NextDouble() < 0.5) FlipBit(randomized, i);
            }
        ...
        public int CompareTo(object obj) {
            if (obj is ID) {
                // Compare as ID.
                if (this < (ID) obj) return -1;
                if (this == (ID) obj) return 0;
                return 1;
            }
