    ...
    public class BucketList {
        ...
        public Contact Blocker(ID toAdd) {
            ...
            lock (buckets[bucket]) {
                ...
                if (buckets[bucket].Count < BUCKET_SIZE) return null;
        ...
        public void Put(Contact toAdd) {
            if (toAdd == null) return; // Don't be silly.
        ...
        public Contact Get(ID toGet) {
            ...
            lock (buckets[bucket]) {
                ...
                for (int i = 0; i < buckets[bucket].Count; i++) {
                    if (buckets[bucket][i].GetID() == toGet) return buckets[bucket][i];
                }
            }
            return null;
        }
            ...
            for (int i = 0; i < NUM_BUCKETS; i++) {
                lock (buckets[i]) {
                    for (int j = 0; j < buckets[i].Count; j++) {
                        ...
                        if (applicant.GetID() == excluded) continue;
        ...
        public IList<ID> IDsForRefresh(TimeSpan tooOld) {
            ...
            lock (accessTimes) {
                for (int i = 0; i < NUM_BUCKETS; i++) {
                    if (DateTime.Now > accessTimes[i].Add(tooOld)) {
                        // Bucket is old
