...
namespace RuntimeEnvironment.Implementation
{
    public class RuntimeEnvironmentComponent : IRuntimeEnvironment
    {
        ...
        (Guid simulationId, TModelDescription model, ICollection<TNodeInformation> layerContainerNodes,
            int? nrOfTicks = null, string scenarioConfigName = "SimConfig.json", string resultConfigId = "",
            bool startPaused = false, ILayerContainerFacade layerContainer = null)
        {
            _runtimeEnvironmentUseCase.StartWithModel(simulationId, model, layerContainerNodes, nrOfTicks,
                scenarioConfigName, resultConfigId, startPaused, layerContainer);
        }
