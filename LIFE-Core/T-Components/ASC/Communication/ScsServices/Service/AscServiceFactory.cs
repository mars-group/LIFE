...
namespace ASC.Communication.ScsServices.Service
{
    ...
    {
        private static readonly ConcurrentDictionary<string, IAscServiceApplication> ServiceAppDictionary =
            new ConcurrentDictionary<string, IAscServiceApplication>();
        ...
        public static IAscServiceApplication CreateService(int port, string multicastGroup, string typeOfService)
        {
            ...
            serviceApp = new AscServiceApplication(
                AcsServerFactory.CreateServer(AscEndPoint.CreateEndPoint(port, multicastGroup)));
