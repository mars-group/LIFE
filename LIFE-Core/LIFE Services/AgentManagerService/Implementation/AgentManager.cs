...
namespace LIFE.Services.AgentManagerService.Implementation {
  ...
  public class AgentManager<T> : IAgentManager<T> where T : IAgent {
    ...
    public IDictionary<Guid, T> GetAgentsByAgentInitConfig(AgentInitConfig agentInitConfig,
      RegisterAgent registerAgentHandle, UnregisterAgent unregisterAgentHandle, List<ILayer> additionalLayerDependencies,
        ...
        foreach (var neededParam in neededParameters) {
          ...
          if (environmentType.GetTypeInfo().IsAssignableFrom(neededParam.ParameterType)) {
            ...
            actualParameters.Add(
              additionalLayerDependencies.First(l => neededParam.ParameterType.GetTypeInfo().IsInstanceOfType(l)));
          }
              ...
              if (param.MappingType == MappingType.ValueParameterMapping) {
                ...
                if ((paramType != typeof(string)) && ((paramType == null) || !paramType.GetTypeInfo().IsPrimitive))
                  throw new ParameterMustBePrimitiveException("The parameter " + param.Name +
                                                              " must be a primitive C# type. But was: " + paramType.Name);
