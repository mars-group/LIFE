...
namespace PhoneBookServer {
    ...
    /// </summary>
    internal class PhoneBookService : ScsService, IPhoneBookService {
        /// <summary>
        ...
        public void AddPerson(PhoneBookRecord recordToAdd) {
            if (recordToAdd == null) throw new ArgumentNullException("recordToAdd");
        ...
        /// <returns>
        ///     True, if a person is deleted,
        ///     false if person is not found
        /// </returns>
        public bool DeletePerson(string name) {
            if (!_records.ContainsKey(name)) return false;
        ...
        ///     Name might not fully match, it can be a part of person's name
        /// </param>
        ...
        public PhoneBookRecord FindPerson(string name) {
            ...
            if (_records.ContainsKey(name)) return _records[name];

            //Search all records to check if there
            //is a name string that contains given name
            foreach (var record in _records) {
                if (record.Key.ToLower().Contains(name.ToLower())) return record.Value;
            }
